...
export default function VelocityMarquee({ text }: { text: string }) {
  ...
  return (
    <div className="overflow-hidden whitespace-nowrap border-y border-purple-500/20 py-5 sm:py-6">
      <div ref={trackRef} className="inline-flex will-change-transform">
        {[0, 1].map((i) => (
          <span
            key={i}
            className="text-5xl sm:text-8xl font-bold uppercase tracking-tight px-6 text-transparent bg-clip-text bg-gradient-to-r from-purple-300 via-purple-200 to-blue-300"
          >
            {text}&nbsp;/&nbsp;
          </span>
        ))}
